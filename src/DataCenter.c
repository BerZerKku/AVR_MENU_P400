...
void FParamPrm(unsigned char command)
{
    ...
    case 0x18: { //Время включения (приемник)
        ...
        if ((Rec_buf_data_uart[4] < RangPrm[0][0]) || (Rec_buf_data_uart[4] > RangPrm[0][1])) {
            TmpMass[0] = '?';
            TmpMass[1] = '?';
        } else {
    ...
    case 0x19: { //длительность команды (приемник)
        ...
        if ((Rec_buf_data_uart[4] < RangPrm[1][0]) || (Rec_buf_data_uart[4] > RangPrm[1][1])) {
            TmpMass[0] = '?';
            TmpMass[1] = '?';
            TmpMass[2] = '?';
        } else {
    ...
    case 0x1A: { //задержка на выключение (приемник)
        ...
        for (uint8_t i = 0; i < Rec_buf_data_uart[3]; i++) {
            if ((Rec_buf_data_uart[4 + i] < RangPrm[2][0]) || (Rec_buf_data_uart[4 + i] > RangPrm[2][1])) {
                TmpMass[i] = 0xFF;
            } else {
...
void FCurrentState(void)
{
    ...
    for (char i = 0; i < 4; i++) { //сделаем проверку принятого состояния и перепишем значение в массив
...
void FDataTime(void)
{
    ...
    if (((DataLCD[7] - 0x30) + ((DataLCD[6] - 0x30) * 10)) > 99)
        DataError++;
    if (((DataLCD[4] - 0x30) + ((DataLCD[3] - 0x30) * 10)) > 12)
        DataError++;
    if (((DataLCD[1] - 0x30) + ((DataLCD[0] - 0x30) * 10)) > 31)
        DataError++;
    if (((TimeLCD[1] - 0x30) + ((TimeLCD[0] - 0x30) * 10)) > 24)
        TimeError++;
    if (((TimeLCD[4] - 0x30) + ((TimeLCD[3] - 0x30) * 10)) > 60)
        TimeError++;
    if (((TimeLCD[7] - 0x30) + ((TimeLCD[6] - 0x30) * 10)) > 60)
        TimeError++;
...
{ //массив, начальный адрес, множитель, параметр
    unsigned char i;
...
void FCorrParam(void)
{
    ...
    if ((temp1 > 99) || (temp1 < -99) || (temp2 > 90) || temp2 < -90) { //вне диапазона
        ...
        if (sCorrParam[0].Print[0] == '-')
            sCorrParam[0].Corr = 0x10000 - sCorrParam[0].Corr;
    }
    //ток 1 и 2
    for (char i = 1; i < 3; i++) {
...
void FMeasureParam(void)
{
    ...
    if (Rec_buf_data_uart[4] == 0x00) { //пришли все параметры
        ...
        //контрльная частота
        if ((cAutoControl == 4) && (CurrentState[0] < 4)) { //если АК выкл и не тест то Uk всегда 0
...
void FParamGlobal(unsigned char command)
{
    ...
    case 0x37: { //тип удаленного аппарата
    ...
    case 0x39: { // Оптика - "Время перезапуска"
        // ВЧ - параметры ПВЗУ-Е
        uchar tmp;
    ...
    case 0x3A: { //частота (общие)
        ...
        if (Pk_temp < RangGlb[6][0])
            Rec_buf_data_uart[4] = 1;
        else if (Pk_temp > RangGlb[6][1])
            Rec_buf_data_uart[4] = 1;
        ...
        if (Rec_buf_data_uart[4]) {
            ...
            if (Pk_temp < 100) {
                ...
                MenuAllFreq[0] = Pk_temp / 1000 + '0';
                Pk_temp %= 1000;
                MenuAllFreq[1] = Pk_temp / 100 + '0';
                Pk_temp %= 100;
                MenuAllFreq[2] = Pk_temp / 10 + '0';
...
#warning Включена принудительная установка типа аппарата!!!
    ...
    if (Rec_buf_data_uart[8] == 2) {
        ...
        if ((bDef) && (cNumComR1) && (cNumComR2) && (cNumComT)) {
            bAllDevice = true;
        }
