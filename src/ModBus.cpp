...
void BazaModBus::writeinf(unsigned int numcom, unsigned char* mass)
{
    ...
    case 0x06:
    case 0x07: {
        writeparampost(numcom, mass);
    } break; //параметры Поста
    case 0x11:
    case 0x12:
    case 0x13:
    case 0x14:
    case 0x15:
    case 0x16: {
        writeparamprm(numcom, mass);
    } break; //параметры Приемника
    case 0x21:
    case 0x22:
    case 0x23:
    case 0x24:
    case 0x25: {
        writeparamprd(numcom, mass);
    } break; //параметры Передатчика
    case 0x30: {
        writecurrentstate(mass);
    } break; // текущее состояние
    case 0x31: {
        writeglobalcurrentstate(mass);
    } break; // общее текущее состояние
    case 0x32: {
        writedatatime(mass);
    } break; // дата/время
    case 0x34: {
        writemeasureparam(mass);
    } break; //измеряемые параметры
    case 0x35:
    case 0x36:
    case 0x37:
    case 0x38:
    ...
    case 0x3B:
    case 0x3C: {
        writeparamgl(numcom, mass);
    } break; //общие параметры
    //архивы
    case 0xC1: {
        writenumberrecording(mass, 0x00B5);
    } break; //кол-во записей в архиве Поста
    case 0xD1: {
        writenumberrecording(mass, 0x00B6);
    } break; //кол-во записей в архиве Приемника
    case 0xE1: {
        writenumberrecording(mass, 0x00B7);
    } break; //кол-во записей в архиве Передатчика
    case 0xF1: {
        writenumberrecording(mass, 0x00B4);
    } break; //кол-во записей в архиве событий
    case 0xC2:
    case 0xD2:
    case 0xE2:
    case 0xF2: {
        writerecordingarchive(mass);
    } break; //чтение записей из архива событий
    }
};
...
void BazaModBus::writeregister(unsigned char* mass)
{
    ...
    case 0x0015: { //изменение даты/времени
        for (i_mod = 0; i_mod < 6; i_mod++)
            mass[4 + i_mod] = ((DataMass[i_mod + 0x10] / 10) << 4) + (DataMass[i_mod + 0x10] % 10);
...
*/
    ...
    case 0x00B4: {
        NumCom = 0xFA;
        NumByte = 0;
        for (i_mod = 0; i_mod < 129; i_mod++)
            JournalMass[i_mod] = 0xEEEE;
...
void BazaModBus::writeparampost(unsigned int numcom, unsigned char* mass)
{
    switch (numcom) {
    case 0x01: {
        DataMass[0x0050] = mass[4];
    } break; //тип защиты
    case 0x02: {
        DataMass[0x0051] = mass[4] + 1;
    } break; //тип линии
    case 0x03: {
        DataMass[0x0052] = mass[4];
    } break; //допустимое время без манипуляции
    case 0x04: {
        DataMass[0x0053] = mass[4];
    } break; //задержка на линии
    case 0x05: {
        DataMass[0x0054] = mass[4];
    } break; //перекрытие импульсов
    case 0x06: {
        DataMass[0x0055] = mass[4];
    } break; //напряжение порога
    case 0x07: {
        DataMass[0x0056] = mass[4];
    } break; //форсировка
    }
};
...
void BazaModBus::writeparamprm(unsigned int numcom, unsigned char* mass)
{
    switch (numcom) {
    case 0x11: {
        DataMass[0x0060] = mass[4];
    } break; //Время включения
    case 0x12: {
        DataMass[0x0061] = 10 * mass[4];
    } break; //длительность команды
        //    case 0x13:  {DataMass[0x006A+mass[4]-1]=mass[5]*10;} break; //задржка на выключение
    case 0x13: {
        for (i_mod = 0; i_mod < mass[3]; i_mod++)
            DataMass[0x6A + i_mod] = mass[4 + i_mod] * 10;
    } break; //задржка на выключение
    case 0x14: {
        for (i_mod = 0; i_mod < mass[3]; i_mod++)
            DataMass[0x0062 + i_mod] = mass[4 + i_mod];
    } break; //блокировка команд 1-16
        /*
    case 0x14:  {DataMass[0x0062+mass[4]-1]|=mass[5];} break;
*/
    case 0x15: {
        for (i_mod = 0; i_mod < mass[3]; i_mod++)
            DataMass[0x0066 + i_mod] = mass[4 + i_mod];
    } break; //длительные команды
        /*
    case 0x15:  {DataMass[0x0066+mass[4]-1]|=mass[5];} break;
*/
    case 0x16: {
        DataMass[0x008A] = mass[4];
    } break;
    }
};
...
void BazaModBus::writeparamprd(unsigned int numcom, unsigned char* mass)
{
    switch (numcom) {
    case 0x21: {
        DataMass[0x0090] = mass[4];
    } break; //время включения
    case 0x22: {
        DataMass[0x0091] = mass[4] * 10;
    } break; //длительность команды
    case 0x23: {
        DataMass[0x0092] = mass[4] * 10;
    } break; //время на повторное формирование команды
    case 0x24: {
        for (i_mod = 0; i_mod < mass[3]; i_mod++)
            DataMass[0x0093 + i_mod] = mass[4 + i_mod];
    } break; //блокировка команд
        /*
    case 0x24:  {DataMass[0x0093+mass[4]-1]|=mass[5];} break;
*/
    case 0x25: {
        for (i_mod = 0; i_mod < mass[3]; i_mod++)
            DataMass[0x0097 + i_mod] = mass[4 + i_mod];
    } break; //длительные команды
        /*
...
void BazaModBus::writemeasureparam(unsigned char* mass)
{
    switch (mass[4]) {
    case 0x01: {
        DataMass[0x0040] = (mass[5] << 8) + mass[6];
    } break; //I1, мА
    case 0x02: {
        DataMass[0x0041] = (mass[5] << 8) + mass[6];
    } break; //I2, мА
    case 0x03: {
        DataMass[0x0042] = mass[5];
        DataMass[0x0043] = mass[6];
    } break; //U, В
    case 0x04: {
        if (mass[5] < 127)
            DataMass[0x0044] = mass[5]; //Uз
        else
            DataMass[0x0044] = mass[5] + 0xFF00;
    } break;
    case 0x05: {
        if (mass[5] < 127)
            DataMass[0x0045] = mass[5]; //Uкч
        else
            DataMass[0x0045] = mass[5] + 0xFF00;
    } break;
    case 0x06: {
        if (mass[5] < 127)
            DataMass[0x0046] = mass[5]; //Uш
        else
            DataMass[0x0046] = mass[5] + 0xFF00;
    } break;
    case 0x07: {
        DataMass[0x0047] = mass[5];
    } break; //Kov
    case 0x08: {
        DataMass[0x0048] = 0x00;
        DataMass[0x0049] = (mass[5] << 8) + mass[6];
    } break; //Pk
    case 0x00: {
        DataMass[0x0040] = (mass[5] << 8) + mass[6]; //I1, мА
        DataMass[0x0041] = (mass[7] << 8) + mass[8]; //I2, мА
        DataMass[0x0042] = mass[9];
        DataMass[0x0043] = mass[10]; //U, В
        if (mass[11] < 127)
            DataMass[0x0044] = mass[11]; //Uз
        else
            DataMass[0x0044] = mass[11] + 0xFF00;
        if (mass[12] < 127)
            DataMass[0x0045] = mass[12]; //Uкч
        else
            DataMass[0x0045] = mass[12] + 0xFF00;
        if (mass[13] < 127)
            DataMass[0x0046] = mass[13]; //Uш
        else
            DataMass[0x0046] = mass[13] + 0xFF00;
        DataMass[0x0047] = mass[14]; //Kov
        DataMass[0x0048] = 0x00;
        DataMass[0x0049] = (mass[15] << 8) + mass[16]; //Pk
    } break;
    }
};
...
void BazaModBus::writeparamgl(unsigned int numcom, unsigned char* mass)
{
    switch (numcom) {
    case 0x35: {
        DataMass[0x00A0] = mass[4];
    } break; //синхронизация часов
    case 0x36: {
        DataMass[0x00A1] = mass[4];
    } break; //удержание реле команд приемника
    case 0x37: {
        DataMass[0x00A2] = mass[4];
    } break; //удержание реле команд передатчика
    case 0x38: {
        DataMass[0x00A3] = mass[4];
    } break; //адрес аппарата в локальной сети
    case 0x39: {
        DataMass[0x00A4] = mass[4];
    } break; //время перезапуска
    case 0x3A: {
        DataMass[0x00A5] = (mass[4] << 8) + mass[5];
    } break; //частота
    case 0x3B: {
        DataMass[0x00A6] = mass[4];
    } break; //номер аппарата
    case 0x3D: {
        DataMass[0x00A7] = mass[4];
    } break; //контроль выходного сигнала
    }
};
