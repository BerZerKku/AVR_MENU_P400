    ...
    if (GetSostPort1() & HardError) //если произошла ошибка приема
    {
        ...
        case 1: {
            if (Rec_buf_data_uart1[0] == AddressModBus) {
                ...
                //************************************************************************************************************8
                if (CRCSum1 != CRCread) {
                    Tr_buf_data_uart1[2] = Lo(CRCSum1);
                    Tr_buf_data_uart1[3] = Hi(CRCSum1);
                    TransDataInf1(0xFF, 2 - 1);
                } //{EnableReceive1;}//прверка принятого CRC
                else {
                    ...
                    case 0x03: {
                        if ((NumberRegister > 64) || (NumberRegister == 0))
                            ErrorMessageModBus(0x03); //кол-во запрашиваемых данных выходит из допустимого диапазона
                        else {
                            ...
                            if (((AddressStartRegister >= 250) && (AddressStartRegister < 1024)) || ((AddressStartRegister + NumberRegister > 250) && (AddressStartRegister < 1024))) {
                                ...
                                if (AddressStartRegister < 250) { //все кроме архивов
                                ...
                                } else if ((AddressStartRegister >= 0x400) && (AddressStartRegister < 0x2200)) { //архив событий
                                    ReadArch = 1;
                                    ModBusBaza->NumberRecordingArchive(0); //
                                    NumberRec = NumberRegister / 15;
                                    if (NumberRec == 0)
                                        NumberRec++;
                                    if ((15 - (AddressStartRegister - 1024) % 15) < NumberRegister)
                                        NumberRec++;
                                    NumRecStart = (AddressStartRegister - 1024) / 15;
                                    ComArch = 0xF2;
                                    //ErrorMessageModBus(0x03);
                                } else if ((AddressStartRegister >= 0x2400) && (AddressStartRegister < 0x4200)) { //архив Поста
                                    ReadArch = 1;
                                    ModBusBaza->NumberRecordingArchive(0); //
                                    NumberRec = NumberRegister / 15;
                                    if (NumberRec == 0)
                                    ...
                                    if ((15 - StRegister) < NumberRegister)
                                        NumberRec++;
                                    NumRecStart = (AddressStartRegister - 0x2400) / 15;
                                    ComArch = 0xC2;
                                } else if ((AddressStartRegister >= 0x4400) && (AddressStartRegister < 0x6200)) { //архив ПРМ
                                    ReadArch = 1;
                                    ModBusBaza->NumberRecordingArchive(0); //
                                    NumberRec = NumberRegister / 15;
                                    if (NumberRec == 0)
                                    ...
                                    ComArch = 0xD2;
                                } else if ((AddressStartRegister >= 0x6400) && (AddressStartRegister < 0x8200)) { //архив ПРД
                                    ReadArch = 1;
                                    ModBusBaza->NumberRecordingArchive(0); //
                                    NumberRec = NumberRegister / 15;
                                    if (NumberRec == 0)
