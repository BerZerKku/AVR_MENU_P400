...
#define WriteLCDdata(data) \
    ;                      \
    SetRS;                 \
    SetE;                  \
    PortLCD = data;        \
    ClrE;
#define WriteLCDcommand(data) \
    ;                         \
    ClrRS;                    \
    SetE;                     \
    PortLCD = data;           \
...
void LCDprint(unsigned char Line, unsigned char AddressInLine, unsigned char* bufer, unsigned char convers)
{
    ...
    while (bufer[i2] != 0x00) {
        if (convers == 1) {
            if ((bufer[i2] >= 'А') && (bufer[i2] <= 'я'))
                LCDbuf[StartChar + i2] = CodeLetterLCD[bufer[i2] - 192];
            else
                LCDbuf[StartChar + i2] = bufer[i2];
        } else
            LCDbuf[StartChar + i2] = bufer[i2];
        i2++;
    }
...
{
    for (unsigned char i4 = 0; i4 < 80; i4++)
        LCDbuf[i4] = 0x20; //записывем пробел
    i1 = 0;
}

void LCDbufClMenu(void)
{ //очистка 3 нижних строк
    for (unsigned char i4 = 20; i4 < 80; i4++)
        LCDbuf[i4] = 0x20;
}
...
void FuncClearCharLCD(unsigned char Line, unsigned char AddressInLine, unsigned char NumberChar)
{
    ...
    StartChar = (Line - 1) * 20 + AddressInLine - 1;
    for (i2 = 0; i2 < NumberChar; i2++)
        LCDbuf[StartChar++] = 0x20;
}
...
void LCDprintHEX(unsigned char Line, unsigned char AddressInLine, unsigned char CodePrint)
{
    ...
    if (LCDbuf[StartChar] > 9)
        LCDbuf[StartChar] += 0x37;
    else
        LCDbuf[StartChar] += 0x30;
    if (LCDbuf[StartChar + 1] > 9)
        LCDbuf[StartChar + 1] += 0x37;
    else
        LCDbuf[StartChar + 1] += 0x30;
}
...
void LCDprintBits(unsigned char Line, unsigned char AddressInLine, unsigned char CodePrint)
{
    ...
    do {
        ...
        if (LCDbuf[StartChar] != 0x30)
            LCDbuf[StartChar] = 0x31;
        StartChar++;
        temp = temp >> 1;
    } while (temp != 0);
}
...
void LCDprintf(unsigned char Line, unsigned char AddressInLine, unsigned __flash char* buferf, unsigned char convers)
{
    ...
    while (buferf[i2] != 0x00) {
        if (convers == 1) {
            if ((buferf[i2] >= 'А') && (buferf[i2] <= 'я'))
                LCDbuf[StartChar + i2] = CodeLetterLCD[buferf[i2] - 192];
            else
...
void LCDptinrArchCount(unsigned char AddressInLine, unsigned int Count, unsigned char Shift)
{
    ...
    if (Count == 256) {
        ...
        LCDbuf[AddressInLine++] = '?'; //ошибочное значение

    LCDbuf[AddressInLine] = ')';

    for (cc = (AddressInLine - cc); cc < 8; cc++)
        LCDbuf[++AddressInLine] = ' '; //сотрем незадействованные ячейки
}
...
void LCDprintTimeAK(unsigned char AK, uchar dev, unsigned char Num, unsigned char* Time)
{
    ...
    case 0: // АВАНТ
    {
        ...
        if (cNumLine == 2) {
            if (Num == '2')
                time2 += 30; //если 2 аппарат, добавим 30 секунд
        } else if (cNumLine == 3) {
            if (Num == '2')
                time2 += 20;
            else if (Num == '3')
        ...
        if (time2 > 59) {
            time2 -= 60;
            time1++;
        } //если 60 сек, то добавим 1 мин и обнулим секунды
...
void LCDprintUkDef(unsigned char AKdecrease)
{
    ...
    LCDbuf[j++] = CodeLetterLCD['д' - 192];
    ; //'д'
    LCDbuf[j++] = CodeLetterLCD['Б' - 192];
    ; //'Б'
