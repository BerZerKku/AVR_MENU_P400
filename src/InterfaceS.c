...
#define StartTimer0 \
    TCNT0 = 5;      \
    TCCR0 = 0x04; //запуск таймера 0
#define InitTimer0 \
    TIFR &= 0xFD;  \
    TIMSK |= (1 << TOIE0); //инициализация таймера0
...
__interrupt void UART_RX_interrupt(void)
{
    ...
    temp_rec = UDR0;
    if (UCSR0A & (1 << FE1))
        return; //FE-ошибка кадра, OVR - переполнение данных (флаги)
    ...
    case 3: {
        ...
        if (temp_rec > (MaxLenghtRecBuf - 5))
            cCurrLen0 = 0; //проверка на макс кол-во данных
        else
            cCurrLen0 = 4;
...
__interrupt void Timer0_overflowed_interrupt(void)
{
    ...
    if (DataPause1 >= MaxPause1) { //если насчитали 3 "пустых" бита, то ...
        if ((StartRec1 == 1) || (Sost1 & HardError)) { //если прием сообщения уже шел, считаем что прием завершен
            ...
            DataPause1 = MaxPause; //если же сообщение не принималось, остановим счет, этого уже вполне достаточно
    } else
        DataPause1++;
...
__interrupt void UART1_RX_interrupt(void)
{
    ...
    if ((StartRec1 == 1) && (DataPause1 < MaxPause)) //если прием уже идет, проверяем интервал ожидания, для определения окончания посылки
    {
        if (RecByteCounter1 > (MaxLenghtRecBuf - 1))
            Sost1 |= HardError; //если буфер переполнен, то пишем ошибку
        else
            Rec_buf_data_uart1[RecByteCounter1++] = temp_rec;
